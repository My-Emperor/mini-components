    ...
    Component({
      methods: {
        ...
        changeBarrageType(e) {
          ...
          if (type) {
            this.canBarrageSupport(id,tunnelid,bulletid);
          } else {
            this.setBarrageSupport(id,tunnelid,bulletid);
          }
        },
        //点赞弹幕
        setBarrageSupport(id,tunnelid,bulletid) {
          if(changeFlag){
            return;
          }
          changeFlag = true;
          videoModel.setBarrageSupport(id).then(res => {
            if (res && res.code == 0) {
              this.barrage.changeLike({
                tunnelId: tunnelid,
                bulletId: bulletid
              });
            }
        ...
        //取消点赞弹幕
        canBarrageSupport(id,tunnelid,bulletid) {
          if(changeFlag){
            return;
          }
          changeFlag = true;
          videoModel.canBarrageSupport(id).then(res => {
            if (res && res.code == 0) {
              this.barrage.changeNoLike({
                tunnelId: tunnelid,
                bulletId: bulletid
              });
            }
        ...
        reportBarrage(e) {
          ...
          //   return;
          // }
          this.barrage.changeReport({
            tunnelId: tunnelid,
            bulletId: bulletid
          });
          wx.navigateTo({
            url: `/pages/complaint/index?id=${id}&reportType=barrage`,
            fail(e) {
              wx.switchTab({
                url: url,
              })
  ...
  (function (module, exports, __webpack_require__) {
    ...
    var Barrage = function () {
      ...
      Barrage.prototype.init = function init() {
        ...
        this.tunnelNum = 3  // Math.floor(this.availableHeight / this.tunnelHeight ); // gradual modify: 修改减少轨道数量 / 2
      ...
      Barrage.prototype.addBullet2Tunnel = function addBullet2Tunnel() {
        ...
        if (tunnel.getIdleBulletNum() === 0) {
          this.removeIdleTunnel(tunnelId);
        }
      };
      ...
      //graudal  modify
      // ------------------------------------------
      Barrage.prototype.changeLike = function changeLike(opt) {
        ...
        var tunnelId = opt.tunnelId;
        var bulletId = opt.bulletId;

        var tunnel = this.tunnels[tunnelId];
        var bullet = tunnel.bullets[bulletId];
        // console.log(bullet.praiseNum);
        // console.log(bullet.praise);

        bullet.praiseStatus = true;
        if(bullet.praiseNum < 1000){
          bullet.praise += 1;
        }
        // console.log(bullet);

        // bullet.paused = !bullet.paused;
        // bullet.paused = true; //graudal  modify
        this.comp.setData((_comp$setData3 = {}, _comp$setData3['tunnels[' + tunnelId + '].bullets[' + bulletId + ']'] = bullet, _comp$setData3));

      };


      Barrage.prototype.changeNoLike = function changeNoLike(opt) {
      ...
      Barrage.prototype.changeReport = function changeReport(opt) {
        var _comp$setData3;
